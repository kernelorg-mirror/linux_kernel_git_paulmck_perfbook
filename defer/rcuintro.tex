...
\setlnpref{C}
To prevent this transformation, use \co{rcu_dereference()} as shown on
\clnrefrange{C1}{C3} of this listing, to inform the compiler of possible
concurrent updates to \co{gp}.
\end{fcvref}
...
\cref{sec:toolsoftrade:Shared-Variable Shenanigans}
to heart, \co{smp_store_release()} (or, as noted earlier,
\co{rcu_assign_pointer()}) is used to \co{NULL} the pointer, thus moving
from the first row to the second in the figure.
...
hundred API members.
However, this section will confine itself to the six core RCU API members
shown in
\cref{tab:defer:Core RCU API},
which suffices for the examples in the upcoming sections.
...
\cellcolor{white}&
	\tco{call_rcu()} &
		Asynchronous counterpart to \co{synchronize_rcu()}:
		Once all pre-exiting RCU read-side critical sections
		have completed, pass the specified value to the specified
		function. \\
&
...
We are now ready to continue this introduction to RCU with the most
fundamental of these core RCU operations, \co{synchronize_rcu()} and
\co{call_rcu()}, each of which waits for pre-existing readers.
